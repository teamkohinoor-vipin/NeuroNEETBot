...
async def unmatched_callback(update: Update, context):
    """Fallback for any unmatched callback queries"""
    logger.warning(f"❓ Unmatched callback: {update.callback_query.data}")
    await update.callback_query.answer(
        "This button is not available right now. Please try /start again."
    )
...
def main():
    ...
    # ================= ADMIN CALLBACK (IMPORTANT: BEFORE CONVERSATION) =================
    ...
    # Clean polling (avoid 409 conflict)
