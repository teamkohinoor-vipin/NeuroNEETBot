...
async def next_action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "next_q":
        ...
        admin_text = (
            f"New question batch from @{update.effective_user.username} (ID: {update.effective_user.id})\n"
        ...
        await query.edit_message_text("Batch submitted for admin approval. You'll be notified once reviewed.")
